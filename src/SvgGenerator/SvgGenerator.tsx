...
import styles from './SvgGenerator.module.scss';
...
const SvgGenerator: React.FC<SvgGeneratorProps> = ({ id, color, width, height }) => {
    switch (id) {
        case 'pawn':
            return (
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className={`${styles['figure-icon']} ${styles[color]}`}
                    viewBox="0 0 100 100"
                    height={`${height}px`}
                    width={`${width}px`}
                    id={`${id}-${color}`}>
